...
for name in samples.keys():
    for leg in legs:
        ...
        for c in addcuts:
            ...
            fitters[Nj].jetDoublePeak('model','x')
            histo = plotter.drawTH1("jj_"+leg+"_softDrop_mass",options.cut+c,"1",80,options.mini,options.maxi)
        ...
        print "plot "+args[0]+'/'+samples[name]+'.root'
        plotter=TreePlotter(args[0]+'/'+samples[name]+'.root','tree')
        plotter.addCorrectionFactor('genWeight','tree')
        plotter.addCorrectionFactor('puWeight','tree')
        ...
        histo = plotter.drawTH1("jj_"+leg+"_softDrop_mass",options.cut+"*(jj_LV_mass>=1000 && jj_LV_mass<7000)","1",80,55,215)
