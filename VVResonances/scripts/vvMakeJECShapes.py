...
def fitGauss(htmp,mass):
    ...
    gauss_refined  = ROOT.TF1("gauss" ,"gaus",mean-1.5*sigma,mean+1.5*sigma)  
...
for mass in sorted(samples.keys()):
    ...
    c.cd(1)
    htmp.SetTitle("")
    cmaximum = 1800#htmp.Integral()*0.06
    x = 800
    y = 1600
    if mass == 4500:
        cmaximum = 2600
    ...
    htmp.SetMaximum(cmaximum)
    htmp.GetXaxis().SetTitle("m_{jj}")
    ...
    legend2.AddEntry(htmp_Down,"JER down","l")
    legend2.AddEntry(htmp,"nominal" ,"l")
    legend2.AddEntry(htmp_Up, "JER up","l")
    text = ROOT.TLatex()
    
    
    text.DrawLatex(x,y,"#color[2]{#mu ="+str(round(resDown[0],2))+" #sigma = "+str(round(resDown[1],2))+" }; #mu ="+str(round(res[0],2))+" #sigma = "+str(round(res[1],2))+"  ;#color[4]{ #mu ="+str(round(resUp[0],2))+" #sigma = "+str(round(resUp[1],2))+" }" )
    ...
    JERhtmp.SetMaximum(cmaximum)
    JERhtmp.GetXaxis().SetTitle("m_{jj}")
    JERhtmp.GetYaxis().SetTitle("events")
    JERhtmp.GetYaxis().SetTitleOffset(1.6)
    JERhtmp.SetTitle("")
    JERhtmp.Draw("")
    JERhtmp_Up.SetLineColor(ROOT.kBlue)
    JERhtmp_Up.Draw("same")
    JERhtmp_Down.SetLineColor(ROOT.kRed)
    JERhtmp_Down.Draw("same")
    func1 = JERhtmp.GetFunction("gauss")
    func1.SetLineColor(ROOT.kBlack)
    func1.Draw("same")
    func2 = JERhtmp_Up.GetFunction("gauss")
    func2.SetLineColor(ROOT.kBlue)
    func2.Draw("same")
    func3 = JERhtmp_Down.GetFunction("gauss")
    func3.SetLineColor(ROOT.kRed)
    func3.Draw("same")
