...
def MakePlots(histos,hdata,axis,nBins,options):
    ...
    if options.zrange == '0,-1': zrange = '1126,5000'
    if axis=='z':
...
def doZprojection(pdfs,data,norm_nonres,norm_res,norm_s,Binslowedge,Bins_redux,binWidths,workspace,options):
    ...
    for i in range(0,len(pdfs)):
	   ...
	   if "Wjet" in str(pdfs[i].GetName()):
	     ...
	     h[i].Fill(zv,lv[i][zv]*(norm_res[0]+norm_res[1]))
...
def doXprojection(pdfs,data,norm_nonres,norm_res,norm_s,Binslowedge,Bins_redux,binWidths,workspace,options):
    ...
    for i in range(0,len(pdfs)):
	    ...
	    if "Vjet" in str(pdfs[i].GetName()): h[i].Fill(key,value*(norm_res[0]+norm_res[1]))
