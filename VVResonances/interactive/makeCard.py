...
vtag_pt_dependence = {'HPHP':'(1+0.31/(1+exp(-0.03*(MH/2-406))))*(1+0.31/(1+exp(-0.03*(MH/2-406))))','HPLP':'(1+0.31/(1+exp(-0.03*(MH/2-406))))*(1-0.12*log(MH/538))','LPLP':'(1+0.12*log(MH/538))*(1+0.12*log(MH/538))'}
...
for sig in signals:
  ...
  for dataset in datasets:
    ...
    for p in purities:
      ...
      if p =="LPLP":
          rootFile="/portal/ekpbms2/home/dschaefer/DiBoson3D/forBiasTests/LPLP/"
      ...
      pseudodata="pythia"
      workspaceName = workspaceName.replace(".root","_pseudodata"+pseudodata+".root")
      ...
      if p!="LPLP":
            ...
            card.addSystematic("CMS_VV_JJ_Wjets_norm_"+p,"lnN",{'Wjets':1.2})
      ...
      card.addSystematic("CMS_pdf","lnN",{'%s'%sig:1.03})
  ...
  combo_workspace=combo_workspace.replace(".root","_pseudodata"+pseudodata+".root")
  os.system('rm %s'%combo_card)
  cmd+=' >> %s'%combo_card
  print cmd
