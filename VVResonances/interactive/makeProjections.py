...
def makeProjections(histo,range1,range2,axis,label):
    ...
    if axis== "z":
        ...
        p.GetXaxis().SetTitle("Softdrop m_{jj} [GeV]")
        p.GetYaxis().SetTitleOffset(1.7)
        p.GetYaxis().SetTitle("A.U.")
        p.GetXaxis().SetTitleOffset(1.2)
        p.GetXaxis().SetTitleSize(0.04)
